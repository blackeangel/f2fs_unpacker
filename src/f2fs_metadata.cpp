...
{
    // Count entries with labels that came from actual security.selinux xattrs.
    // Labels added by enrichFromFileContextsText() are NOT counted here —
    // those are used for display only; the embedded file is copied verbatim.
    long xattr_labels = 0;
    for (const auto& m : entries_)
        if (m.selinux_from_xattr) ++xattr_labels;
    ...
    if (xattr_labels > 0) {
        // ── Path 1: labels came from security.selinux xattrs ────────────────
        // Write one exact-path entry per labelled file.  This is what was
        // actually baked into the image.
        fputs("# file_contexts — generated by f2fs_extract\n"
              "# Source: security.selinux xattrs found in the image.\n"
              "# format: path_regex  selinux_label\n", f);

        std::vector<const FileMetadata*> sorted;
        for (const auto& m : entries_)
            if (m.selinux_from_xattr) sorted.push_back(&m);
        std::sort(sorted.begin(), sorted.end(),
                  [](const FileMetadata* a, const FileMetadata* b){
                      return a->path < b->path;
                  });
    ...
    if (!embedded_ctx_files_.empty()) {
        // ── Path 2: no xattr labels; copy the embedded text file verbatim ───
        // Android stores per-partition SELinux labels in a text file inside
        // the partition itself (plat_file_contexts, vendor_file_contexts, …).
        // For repacking with `sload.f2fs -s`, ALL patterns are needed — the
        // 80 %+ that are regex wildcards as well as the exact-path entries.
        // Copying verbatim preserves everything sload.f2fs expects.
        const std::string& src_path = embedded_ctx_files_.front();
        FILE* src = fopen(src_path.c_str(), "rb");
        if (src) {
            fprintf(stdout, "[INF] file_contexts: copying %s verbatim "
                    "(no xattr labels in image)\n", src_path.c_str());
            fputs("# file_contexts — generated by f2fs_extract\n"
                  "# Source: embedded text context file (copied verbatim).\n"
                  "#         All regex patterns preserved for sload.f2fs -s.\n",
                  f);
            char buf[4096];
            size_t n;
    ...
    // ── Path 3: no labels of any kind ───────────────────────────────────────
    fputs("# file_contexts — generated by f2fs_extract\n"
          "# No security.selinux xattrs found and no embedded context file.\n"
          "# Pass no -s flag to sload.f2fs when repacking this partition.\n", f);
    fclose(f);
    return true;
}
...
bool MetadataWriter::enrichFromFileContextsText(const std::string& txt_path)
{
    ...
    // Always track the file as a candidate for verbatim copy even if we got
    // zero exact-path matches (the file may be all-regex patterns).
    embedded_ctx_files_.push_back(txt_path);
    return true;
}
